        ...
        <v-card class="mt-4">
          <v-card-text>
            <v-form v-model="valid" ref="form" lazy-validation>
              <v-text-field label="Username" outlined v-model="form.email" :rules="emailRule" />
              <v-text-field v-model="form.password" :append-icon="show1 ? 'mdi-eye' : 'mdi-eye-off'"
                ...
                label="Password" hint="At least 8 characters" counter outlined @click:append="show1 = !show1" />
              <v-btn class="indigo darken-4 white--text mb-4" large depressed block @click="submit" :loading="loading"
                :disabled="loading">Sign In</v-btn>
                <v-btn block class="text-none" @click="$router.push('/register')">
              Register
            </v-btn>
            </v-form>
